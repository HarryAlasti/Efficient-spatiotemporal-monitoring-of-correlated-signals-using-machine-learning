...
jmp = 3;
step = 0.001;
RPT = 2;   % # of averaging in tracking the spatial signal
Npnt = 20;
rw = 0;                   % rw = 0 for no reward. rw= 1 for full reward.
grad_type = 1;        % grad_type = 1 for aggressive gradient, 
self = 1;                % 1 for using previous step as reference in calculation of error
nFlag = 0;             % 0 for norm-2 and 1 for norm-1
       ...
       [p,xx] = hist(ZI(:),500);
        ...
        if (pnt > 1)
           if (grad_type == 1)
                ...
                Delta = Delta * (1 + mu*(span_factor/(1+span_factor))^0.5 *(error - error_old)/(error + error_old));        
            end
        end
...
%Delta_done = Delta;
Delta_done = Delta_unif_avg(end);

figure(20); mesh(XI,YI,ZI); grid on;
%figure(19); mesh(XI,YI,Data); grid on;

figure(25); plot(Slevel(1,:), Span_u/FULL_SPAN, 'g-s'); ylabel('Signal range span (%)'); xlabel('# of levels');grid on;
...
figure(23); plot(Slevel(1,:), 20*log10(Error_learn_unif),'-sg');grid on; ylabel('Iteration Error (dB)'); xlabel('# of Levels');
pause(1);
